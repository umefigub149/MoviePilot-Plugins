...
- **支持电影和电视剧**订阅自动追更
- **三重搜索源**：Nullbr（TMDB ID 精准匹配）+ HDHive（浏览器/Cookie 模式无需 OpenAPI，OpenAPI 可选）+ PanSou（网盘聚合搜索）
...
**HDHive**（影巢）:
- 推荐使用浏览器模式：无需 OpenAPI，配置 HDHive Cookie 即可恢复旧版自动搜索115资源路径
- 也可配置 HDHive 用户名/密码作为 Cookie失效时的兜底登录
- OpenAPI 模式仍保留为可选高级模式，需要 Client ID、应用 Secret、回调地址、授权码和用户 Token
- 搜索到115分享后继续复用本插件现有缺集匹配、自动转存、订阅状态更新流程
...
2. 配置 115 Cookie
3. 配置至少一个搜索源（推荐 HDHive 浏览器模式：填写 HDHive Cookie，无需 OpenAPI；也可使用 Nullbr 或 PanSou）
