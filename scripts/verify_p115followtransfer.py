...
assert "演练模式（只记录日志，不真正入队）" in source
...
assert "_path_help_message" in source and "无法解析目录" in source
assert "与 115strm助手网盘管理的区别" in readme
assert "演练模式" in readme
...
assert "v1.0.2" in plugin["history"] and "v1.0.1" in plugin["history"] and "v1.0.0" in plugin["history"]
