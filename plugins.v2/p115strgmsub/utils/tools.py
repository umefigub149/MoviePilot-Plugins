...
def estimate_resource_episode_span(title: str) -> int:
    """
    从资源标题粗估覆盖集数，用于 HDHive/分享资源排序。

    规则（命中即返回，分数越大表示覆盖越多）：
    1. E01-E16 / S01E01-E16 / 01-16 / 1-16 等区间 → end-start+1
    2. 更新至第N集 / 更新至N → N
    3. 仅出现单集 E05 / 第5集 → 1
    4. 无法识别 → 0

    说明：这是标题级启发式，不是真实文件列表扫描。
    """
    import re
    ...
    # 区间：S01E01-E16 / E01-E16 / 01-16 / 1-16 / 第1-16集
    range_patterns = [
        r"[Ss]?\d{0,2}[Ee](\d{1,3})\s*[-~～到至]\s*[Ee]?(\d{1,3})",
        r"(?<!\d)(\d{1,3})\s*[-~～到至]\s*(\d{1,3})(?!\d)",
        r"第\s*(\d{1,3})\s*[-~～到至]\s*(\d{1,3})\s*集",
    ]
    best = 0
    ...
    # 更新至第N集 / 更新至N
    m = re.search(r"更新至\s*第?\s*(\d{1,3})\s*集?", text, re.IGNORECASE)
    if m:
        try:
            n = int(m.group(1))
            if 0 < n < 1000:
    ...
    # 单集：S01E05 / 第5集
    if re.search(r"[Ee](\d{1,3})(?!\s*[-~～到至])", text) or re.search(r"第\s*\d{1,3}\s*集", text):
        return 1

    return 0


def sort_share_resources(resources: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    统一资源排序：
    1. 免费优先（need_unlock=False 在前）
    2. 标题估算覆盖集数多的优先
    3. 付费资源积分低的优先
    4. 官方优先
    """
    ...
    def _key(r: Dict[str, Any]):
        ...
        span = r.get("episode_span")
        if span is None:
            span = estimate_resource_episode_span(title)
            r["episode_span"] = span
        need_unlock = bool(r.get("need_unlock", False))
        unlock_points = int(r.get("unlock_points") or 0)
        is_official = bool(r.get("is_official", False))
        return (
            need_unlock,                 # False 免费优先
            -int(span or 0),             # 覆盖集数多优先
            unlock_points if need_unlock else 0,  # 付费积分低优先
            not is_official,             # 官方优先
        )
